import type { Repo, SessionSummary, SessionDetail, SseEvent, PrResult, VerifyResult, ProxyConfig, CheckpointAction, BatchResult, BrowserStatus, FilesListing, FileDetail } from "./types";
...
export const getFiles = (id: string) => j<FilesListing>(`/api/sessions/${id}/files`);
export const getFile = (id: string, path: string) =>
  j<FileDetail>(`/api/sessions/${id}/file?path=${encodeURIComponent(path)}`);
